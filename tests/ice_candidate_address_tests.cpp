...
TEST_F(IceCandidateAddressTest, ipv4_from_string_test) {
    ...
    ASSERT_TRUE(ipv4_rv.unwrap().as_ip_address().has_value());
    EXPECT_EQ(ipv4_rv.unwrap().as_ip_address()->get(), net::ip::Address::from_string(ipv4_str).unwrap());
}
...
TEST_F(IceCandidateAddressTest, ipv6_from_string_test) {
    ...
    ASSERT_TRUE(ipv6_rv.unwrap().as_ip_address().has_value());
    EXPECT_EQ(ipv6_rv.unwrap().as_ip_address()->get(), net::ip::Address::from_string(ipv6_str).unwrap());
}
...
TEST_F(IceCandidateAddressTest, fqdn_from_string_test) {
    ...
    ASSERT_TRUE(fqdn_rv.unwrap().as_fqdn().has_value());
    EXPECT_EQ(fqdn_rv.unwrap().as_fqdn()->get(), net::Fqdn::from_string(fqdn_str).unwrap());
}
